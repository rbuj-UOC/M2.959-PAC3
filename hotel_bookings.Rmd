...
p <- ggplot(
    ...
    theme_minimal() +
    theme(
        plot.title = element_text(hjust = 0, size = 48),
...
p <- ggplot(revenue_by_meal_plan, aes(
    ...
    geom_bar(stat = "identity", position = "stack", alpha = 0.8, color = "white", size = 0.2) +
    scale_y_continuous(labels = scales::label_number(scale = 1, big.mark = ".")) +
    theme_minimal() +
    theme(
    ...
    scale_fill_manual(
        ...
        labels = c(
            "Undefined" = "0. No definit",
            "SC" = "0. Sense menjar",
            "BB" = "1. Esmorzar",
            "HB" = "2. Mitja pensió",
            "FB" = "3. Pensió completa"
        ),
        values = c(
            "Undefined" = "#cbcbcb",
            "SC" = "#dad9d9",
...
ggplot(lead_time_cancellation, aes(x = lead_time_category, y = cancellation_rate, fill = lead_time_category)) +
    ...
    geom_text(aes(label = paste0(cancellation_rate, "%")), vjust = -0.5, size = 5) +
    labs(
        title = "Cancel·lacions segons temps d'anticipació de reserva",
...
ggplot(
    ...
    coord_flip() +
    labs(
        title = "Durada d'estada: Portuguesos vs. Internacionals",
        x = "Origen",
        y = "Nits d'estada"
    ) +
    theme_minimal() +
